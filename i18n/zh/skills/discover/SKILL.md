...
- **graph API 的 list endpoint 字段较扁平**：S2 的 list endpoint（`/search`、`/citations`、`/references`、`/recommendations/*`）拒绝嵌套 selector —— 没有 `authors.hIndex`，没有 `tldr`。只有 `/paper/{id}` 才带全套字段。anchor 模式下大部分候选的 rationale 会缺 h-index 与 TLDR，这是 S2 的真实约束，不是 bug。
