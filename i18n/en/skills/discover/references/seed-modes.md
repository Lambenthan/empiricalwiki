...
If `wiki/papers/` is empty or no papers carry an `arxiv_id` frontmatter field, wiki mode cannot run. Tell the user the wiki is too sparse and suggest topic mode (or `/init`).
